...
def build_pipeline(args, model_config, device):
    model_configs = [
...
def build_episode_view(dataset):
    episode_records = {}
    for row in dataset.data:
        task = row.get("task", "")
        source_episode_index = int(row.get("source_episode_index", row["episode_index"]))
        episode_key = (task, source_episode_index)
        raw_length = int(row.get("raw_length", row["length"]))

        if episode_key in episode_records:
            existing = episode_records[episode_key]
            for key in ("episode_index", "video", "action"):
                if existing[key] != row[key]:
                    raise ValueError(f"Inconsistent {key} for episode {episode_key}")
            if int(existing["raw_length"]) != raw_length:
                raise ValueError(f"Inconsistent raw_length for episode {episode_key}")
            continue

        record = row.copy()
        record["start_frame"] = 0
        record["end_frame"] = raw_length - 1
        record["length"] = raw_length
        record["raw_length"] = raw_length
        record.pop("window_index", None)
        episode_records[episode_key] = record

    dataset.data = list(episode_records.values())
    return dataset
...
def main():
    ...
    dataset = build_episode_view(build_infer_dataset(args))
    start_index = int(args.start_index)
    stop_index = len(dataset)
    if args.max_samples:
        stop_index = min(stop_index, start_index + int(args.max_samples))
    ...
    print(
        f"[distributed] rank={accelerator.process_index}/{accelerator.num_processes} "
        f"device={accelerator.device} episodes={len(local_indices)} indices={local_indices}"
    )

    pipe = build_pipeline(args, model_config, device=accelerator.device)
    ...
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        print(f"[complete] generated_episodes={len(selected_indices)} output_path={args.output_path}")
