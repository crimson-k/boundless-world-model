...
def encode_video(jpegs, path, fps):
    ...
    process = subprocess.Popen(
        [
            "ffmpeg", "-loglevel", "error", "-y", "-f", "image2pipe",
            "-framerate", str(fps), "-c:v", "mjpeg",
            "-i", "-", "-vf", "colorchannelmixer=rr=0:rb=1:br=1:bb=0",
            "-c:v", "libaom-av1", "-cpu-used", "8", "-row-mt", "1",
            "-pix_fmt", "yuv420p", "-crf", "30", "-g", "2", str(tmp_path),
        ],
    ...
    try:
        ...
        if returncode != 0:
            ...
            message = f"ffmpeg failed with exit code {returncode}: {path}"
            if detail:
                message += f"\n{detail}"
            raise RuntimeError(message) from pipe_error
...
def _episode_paths(output_dir, episode_idx, cameras):
...
def _episode_outputs_complete(output_dir, episode_idx, cameras, num_frames):
    parquet_path, video_paths = _episode_paths(output_dir, episode_idx, cameras)
    try:
        if pq.ParquetFile(parquet_path).metadata.num_rows != num_frames:
            return False
...
def _write_episode(episode_idx, ep, cameras, global_offset, fps, output_dir,
    ...
    if write_outputs:
        ...
        parquet_path, video_paths = _episode_paths(
            output_dir, episode_idx, cameras,
        )
        parquet_path.parent.mkdir(parents=True, exist_ok=True)
        tmp_parquet_path = parquet_path.with_name(
            f"{parquet_path.stem}.tmp{parquet_path.suffix}",
        )
        tmp_parquet_path.unlink(missing_ok=True)
        pq.write_table(pa.table(cols), tmp_parquet_path)
        tmp_parquet_path.replace(parquet_path)

        for camera, video_path in zip(cameras, video_paths):
...
def _task_is_complete(output_dir, num_episodes, fps, robot_type):
    ...
    if (
        info.get("total_episodes") != num_episodes
        or info.get("fps") != fps
        or info.get("robot_type") != robot_type
    ):
        return False
    ...
    for episode_idx in range(num_episodes):
        ...
        for video_key in video_keys:
            video_path = output_dir / (
                f"videos/chunk-{chunk:03d}/{video_key}/"
                f"episode_{episode_idx:06d}.mp4"
            )
            if not video_path.is_file() or video_path.stat().st_size == 0:
                return False
...
def _read_converted_low_dim(output_dir):
    info = json.loads((output_dir / "meta/info.json").read_text())
    actions = []
    states = []
    for episode_idx in range(info["total_episodes"]):
        parquet_path, _ = _episode_paths(output_dir, episode_idx, [])
        table = pq.read_table(
            parquet_path, columns=["joint_abs", "eef_abs"],
        )
        actions.append(np.asarray(table["joint_abs"].to_pylist()))
        states.append(np.asarray(table["eef_abs"].to_pylist()))
...
def convert_task(task_dir, output_dir, subset, fps, robot_type, workers):
    ...
    completed = [
        _episode_outputs_complete(
            output_dir, i, cameras, frame_counts[i],
        )
        for i in range(len(episodes))
    ]
    if any(completed):
        tqdm.write(
            f"  Resuming: skipping outputs for {sum(completed)}/{len(episodes)} "
            "completed episodes"
        )
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {}
...
def main():
    ...
    for task_dir in tqdm(task_dirs, desc="Tasks", unit="task"):
        ...
        num_episodes = len(list((task_dir / args.subset / "data").glob(
            "episode*.hdf5",
        )))
        if _task_is_complete(
            output_dir, num_episodes, args.fps, args.robot_type,
        ):
            tqdm.write(f"Skipping completed task {task_dir.name}")
            skipped_output_dirs.append(output_dir)
            continue
    ...
    if converted_any or not global_stats_path.exists():
        for output_dir in tqdm(
            skipped_output_dirs, desc="Loading completed stats data",
            unit="task", leave=False,
        ):
            actions, states = _read_converted_low_dim(output_dir)
            global_actions.extend(actions)
            global_states.extend(states)
        ...
        tqdm.write(f"Done. All tasks already completed; global stats unchanged.")
